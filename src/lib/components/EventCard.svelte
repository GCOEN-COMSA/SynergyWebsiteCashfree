...
<div on:click|stopPropagation={onLearnMore} class=" group card mx-auto max-w-xs md:max-w-md lg:max-w-lg bg-base-200 image-full my-8">
    <!-- <figure><img src="{image ? image : 'https://placeimg.com/400/225/tech'}" alt="Shoes" /></figure> -->
    <figure class=""><img src="{event.card_image}" alt="Shoes" class="object-fill" /></figure>
    ...
    <div class="card-body">
      ...
      <div class="card-actions justify-end">
        ...
        {#if !event.hide}
        <button on:click|stopPropagation={onLearnMore} class="visible bottom-0 left-0 w-full btn border-none backdrop-blur-sm bg-opacity-5 !group-hover:bg-opacity-10 absolute rounded-2xl rounded-t-none active:scale-y-90 !scale-x-100">Explore</button>
        {/if}
      </div>
    </div>
  </div>
